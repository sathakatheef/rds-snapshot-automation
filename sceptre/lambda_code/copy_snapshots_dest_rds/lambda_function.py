...
COPY_RETENTION_DAYS = int(os.getenv('COPY_RETENTION_DAYS'))
    ...
    for shared_identifier, shared_attributes in shared_snapshots.items():
        ...
        # Check date
            ...
            if creation_date:
                ...
                # Only copy if it's newer than COPY_RETENTION_DAYS
                if days_difference < COPY_RETENTION_DAYS:
                    ...
                    logger.info('Not copying %s locally. Older than %s days' % (shared_identifier, COPY_RETENTION_DAYS))
